...
import { fetchMenusByCategory } from "@/services/main.services";
import Image from "next/image";
import Link from "next/link";

export default async function MenuByMealTime() {
  const data = await fetchMenusByCategory();
          ...
          >
            <div className="w-full">
              <Image
                ...
                width={1000}
                height={2000}
                className="w-screen h-60 sm:h-96 object-cover rounded-lg shadow-lg hover:scale- transition duration-500 cursor-pointer"
              />
            </div>
            <div className="mt-10 md:-mt-3 grid gap-3  items-start w-full h-min">
              ...
              <div className="flex justify-center items-center">
                <Link
                  href={`/menus/category/${menu?.id}`}
                  className="py-2 px-6 text-center text-primary border-2 border-primary mt-4 hover:bg-primary hover:text-black transition-all tracking-wider font-medium w-max capitalize rounded-lg"
                >
                  show More
                </Link>
              </div>
